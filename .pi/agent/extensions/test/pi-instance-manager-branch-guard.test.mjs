...
test("guardBranchNavigation allows /fork while local turn lock is active", async () => {
  const notices = [];
  const result = await guardBranchNavigation({
    ctx: {
      hasUI: true,
...
test("guardBranchNavigation allows /fork while manager reports a session lock", async () => {
  ...
  const result = await guardBranchNavigation({
    ...
    probeManagerState: async () => ({
      state: {
        activeLocks: [{ sessionId: "s1", owner: "pi-discord-bot:prompt:session=s1", token: "tok-remote" }],
        activeCompactions: [],
        turnQueues: [],
      },
      errorMessage: "",
