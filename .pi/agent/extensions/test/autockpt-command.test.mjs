...
import {
  AUTOCKPT_HELP_TEXT,
  parseAutockptPercent,
  registerAutockptCommand,
} from "../lib/autockpt/autockpt-command.ts";
...
test("parseAutockptPercent validates 0..100 inputs", () => {
  assert.equal(parseAutockptPercent("7.5"), 7.5);
  assert.equal(parseAutockptPercent("", 1), 1);
  assert.equal(parseAutockptPercent("101"), null);
  assert.equal(parseAutockptPercent("abc"), null);
});
...
test("registerAutockptCommand shows shared help text", async () => {
  ...
  assert.deepEqual(ctx.notices.at(-1), { msg: AUTOCKPT_HELP_TEXT, level: "info" });
});
