...
test("formatGitStateLabel keeps a compact plain-text contract with colored numbers", () => {
  const label = formatGitStateLabel({ repoRoot: "/repo", files: 3, additions: 120, deletions: 8 });
  assert.equal(stripAnsi(label), "git 3f +120 -8");
...
test("buildGitStateSignature is stable and explicit", () => {
  ...
  assert.equal(
    buildGitStateSignature({ repoRoot: "/repo", files: 1, additions: 2, deletions: 3 }),
    "/repo\t1\t2\t3",
  );
});
