  ...
  pi.registerCommand("autockpt", {
    ...
    handler: async (args, ctx) => {
      ...
      const lockLine = (() => {
        ...
        const cp = lock.checkpointPath ? ` checkpoint=${lock.checkpointPath}` : "";
        return `compactionLock: pid=${lock.pid} ageSec=${ageSec}${cp}`;
      })();
      ...
      if (a.startsWith("test")) {
        ...
        const next = rest ? Number.parseFloat(rest) : 1;
        if (!Number.isFinite(next) || next < 0 || next > 100) {
