...
import codexFastMode, {
  ...
  resolveCodexFastModeServiceTier,
...
test("codex fast service tier applies only to enabled openai-codex requests", (t) => {
  const previousTier = process.env.PI_CODEX_FAST_MODE_SERVICE_TIER;
  t.after(() => {
    if (previousTier === undefined) delete process.env.PI_CODEX_FAST_MODE_SERVICE_TIER;
    else process.env.PI_CODEX_FAST_MODE_SERVICE_TIER = previousTier;
  });
  delete process.env.PI_CODEX_FAST_MODE_SERVICE_TIER;
  ...
  assert.equal(resolveCodexFastModeServiceTier(), "priority");
  process.env.PI_CODEX_FAST_MODE_SERVICE_TIER = "auto";
  assert.equal(resolveCodexFastModeServiceTier(), "auto");
  process.env.PI_CODEX_FAST_MODE_SERVICE_TIER = "fast";
  assert.throws(() => resolveCodexFastModeServiceTier(), /expected one of auto, default, flex, priority/);
  delete process.env.PI_CODEX_FAST_MODE_SERVICE_TIER;
  ...
  assert.deepEqual(withCodexFastServiceTier({ model: "gpt-5.4" }, "flex"), {
    model: "gpt-5.4",
    service_tier: "flex",
  });
...
test("extension toggles Responses API service_tier=priority and contributes a footer effort suffix", async (t) => {
  ...
  const afterProvider = pi.events.get("after_provider_response")?.[0];
  assert.equal(typeof afterProvider, "function");
  afterProvider({ status: 200, headers: {} }, ctx);
  await command.handler("status", ctx);
  assert.match(ctx.calls.at(-1)?.text ?? "", /service_tier=priority/);
  assert.match(ctx.calls.at(-1)?.text ?? "", /status=200/);
