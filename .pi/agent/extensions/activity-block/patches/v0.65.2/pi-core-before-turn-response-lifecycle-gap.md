# activity-block note (v0.65.2): missing before_turn_response / queued-turn lifecycle patch

Status:
- **Resolved for the current extension implementation**
- Existing saved v0.65.2 patches in this directory are all still necessary
- The missing lifecycle patch now exists as:
  - `pi-core-before-turn-response-lifecycle.patch`

Existing saved v0.65.2 patches:
...
This note remains as historical context for why the third `v0.65.2` patch was added separately instead of being folded into the older patch artifacts.
...
The saved v0.65.2 patch set only covers:
...
## Observable risk on plain upstream v0.65.2 before the new patch

Even if the two saved v0.65.2 patches are applied, the current extension can still drift on queued turns:
...
In short: before the new patch, the block UI could load, but the current per-turn contract was not fully backed by upstream v0.65.2.
...
## Relationship to the existing v0.65.2 patches

Keep these concerns separate:

### Already covered by saved v0.65.2 patches
...
When regenerating a full v0.65.2 patch set for the current extension, do not fold this silently into the older patch notes.
