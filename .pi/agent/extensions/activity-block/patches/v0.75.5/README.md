# activity-block pi-core patches (pi-mono v0.75.5)

Target upstream tag:
- `earendil-works/pi-mono` `v0.75.5` (`89ba72c3`)
...
git apply --unidiff-zero /home/tan/.pi/agent/extensions/activity-block/patches/v0.75.5/pi-core-local-extension-seams.patch
```

Verification used when this stack was created:

```bash
cd /tmp/pi-work/pi-mono-v0755-clean
npm ci
...
```bash
PI_CORE_PATCH_VERSION=0.75.5 bash ~/vault/.pi/scripts/pi/reapply-local-patches.sh
```
