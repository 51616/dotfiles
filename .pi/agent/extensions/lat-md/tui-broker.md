...
The editor override subclasses `CustomEditor` and decorates the final rendered border lines rather than replacing core editing behavior. It forces the user editor border to `#f2cdcd` so core thinking-level updates cannot change the input border color.
