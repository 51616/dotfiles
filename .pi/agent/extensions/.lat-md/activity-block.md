...
- `agent_end` still finalizes the active block when a run fails before `turn_end` is emitted, and completion/abort/error paths retain the terminal snapshot in the dock until the next input instead of clearing it immediately
