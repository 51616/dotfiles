...
- install the live block as the `activity-block-live-dock` `aboveEditor` widget while a turn is active when `ctx.ui.setWidget` is available, hide the matching active transcript message to prevent duplicate UI, keep the terminal dock visible after completion/abort/error until the next user input, and then clear the dock so the transcript block becomes visible as history; contexts without `setWidget` stay in documented transcript-only mode rather than hiding the block
...
- historical transcript suppression stays enabled for the session, live transcript suppression stays enabled until the current active block is finalized on `agent_end`, and the terminal dock stays visible until the next idle `input` event clears it
- `agent_end` still finalizes the active block when a run fails before `turn_end` is emitted, marks any still-running tools as failed with `completedAt = endedAt`, and completion/abort/error paths retain the terminal snapshot in the dock until the next input instead of clearing it immediately
...
- Change `activity-block/index.ts` when the toggle command/shortcut wiring, periodic context-usage refresh, block-spawn timing, live-dock install/clear timing, active transcript hiding, retained terminal dock behavior, zen-mode hiding/toast behavior, run-lifecycle behavior, or `Esc` abort handling is wrong.
